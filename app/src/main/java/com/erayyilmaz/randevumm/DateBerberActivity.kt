...
import com.erayyilmaz.randevumm.databinding.ActivityDateBerberBinding
import com.google.firebase.firestore.FirebaseFirestore
import com.google.firebase.firestore.ktx.firestore
import com.google.firebase.ktx.Firebase

class DateBerberActivity : AppCompatActivity() {
    private lateinit var binding: ActivityDateBerberBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        name = intent.getStringExtra("berberID").toString()
        super.onCreate(savedInstanceState)
        binding = ActivityDateBerberBinding.inflate(layoutInflater)
        ...
        getData()
        controlDate()
        val name = intent.getStringExtra("berberID")
        println("ID: " + name)
    }

    private fun getData() {
        val name = intent.getStringExtra("berberID")
        db.collection("berber").document("${name}").collection("date").get()
    ...
    fun updateDatabase(hour: String, value: Boolean) {
        ...
        db.collection("berber").document(name!!).collection("date").document("datetime").update(hour, value)
            .addOnSuccessListener {
                // Başarıyla güncellendi
                Toast.makeText(this, "Randevunuz oluşturuldu", Toast.LENGTH_SHORT).show()
            }
            .addOnFailureListener { e ->
                // Hata durumunda kullanıcıya bilgi ver
                Toast.makeText(
                    this,
                    "Randevu oluşturulurken hata oluştu: ${e.localizedMessage}",
                    Toast.LENGTH_LONG
                ).show()
            }
    }
    ...
    fun getDate(view: View, hour: String, timeValue: Boolean) {
        if (!timeValue) {
            ...
            val alert = AlertDialog.Builder(this@DateBerberActivity)
